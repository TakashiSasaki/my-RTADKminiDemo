...
public class LEDControl extends ImageButton implements OnClickListener {
	 ...
	 *            The associated context
	 * @param attrs
	 *            The associated AttributeSet for this item
	 */
	...
	/**
	 * Initialized the internal state variables and registers the click listener
	 * for this button
	 */
	private void init() {
		this.on = false;
	...
	/**
	 * Set the user interface handler where this class should send messages to
	 * when a press event occurs
	 * 
	 * @param handler
	 *            If a GUI wants to be notified of this event, it can send it's
	 *            handler here (only supports one handler).
	 */
	...
	public void setState(boolean state) {
		...
		Message ledUpdate = Message.obtain(this.uiHandler,
				UPDATE_OUTPUTPIN_SETTING);
		if (this.uiHandler != null) {
			this.uiHandler.sendMessage(ledUpdate);
		}
